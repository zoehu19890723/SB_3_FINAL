  ...
  <div class="page">
    ...
    <v-content className="page-content">
      <div :class="['summary-card',monthArray.length !== 0?'':'hiddenModule']">
        <div class="monthCard city-card" style="color: black;">
...
<script>
  ...
  export default {
    ...
    mounted:function(){
      ...
      me.$store.dispatch('getFundMonth').then(
        (result) =>{
          if(result.length>0){
            me.monthArray = result;
            me.currentMonth = result[0];
            me.initPicker(result[0]);
            me.$store.dispatch('getFundData',{month : result[0]}).then(
              (result) =>{
