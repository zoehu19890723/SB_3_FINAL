...
<script>
  ...
  export default {
    created:function(){
      this.$store.dispatch('getLikeTypes',{});
    },
    data(){
      return {
    ...
    computed: {
      likeTypes(){
        const typeList = this.$store.state.moduleMy.likeTypes
      ...
      send(){
        ...
        api.postSendLikes(
          ...
          const likeObj = {
            ...
            'likeName':response.body.sent.like_name,
            'likeIcon':response.body.sent.like_icon_url,
            'content': response.body.sent.message,
            'dateDay':response.body.sent.date.split('T')[0],
            'dateTime':response.body.sent.date.split('T')[1]
          }}
           self.$store.commit('setIncreaseLikes',likeObj);
    ...
    locales: {
      en: {
        ...
        loadingText : 'Loading',
        likeType:'Please choose one like',
