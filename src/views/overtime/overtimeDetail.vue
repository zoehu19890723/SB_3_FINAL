  ...
  export default {
    ...
    methods : {
      cancelApply(res){
        ...
        me.$store.dispatch('cancelOTApply',{id : applyId,reson : reson}).then(
          (response) =>{
            if(response.body.status === 1){
              if(response.body.data.status == 1){
                f7.alert('',me.$t('ot-cancel-apply'),()=>{
                  me.$router.back();
                })
              }else{
    ...
    locales: {
      ...
      en: {
        ...
        'refuse-apply' : 'Refuse',
        'approve-apply' : 'Agree',
        'ot-cancel-apply' : 'Cancel leave succeed',
        'real-ot' : 'OverTime Detail',
