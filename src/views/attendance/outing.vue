      ...
      <div class="topPanel">
        <div class="topPanel-1">
          <div class="left-panel">
            ...
            <CompanySelect fieldCode="outCmp" :avaliableArray ="companyArray" :closeAction="changeCompany"></CompanySelect>
          </div>
        </div>
        <div class="work-day">
          <span>{{$t('workDay')}}</span>
      ...
      <div class="attendance-panel">
        ...
        <div :class= "['work', workActive ? 'active' : '',showPunchPanel ? 'showPunchPanel' : '']">
          ...
          <div :class="['attendence-action-panel',(inPosition === 3)? '':'disable']">
            <div class="attendence-action back-color-not-imp" @click="punch(2)">
        ...
        <div :class= "['work', leaveActive ? 'active' : '',showPunchPanel ? '' : 'showPunchPanel']" style="margin-top:16px">
          ...
          <div :class="['attendence-action-panel',(inPosition === 3) ? '':'disable']">
            <div class="attendence-action back-color-not-imp" @click="punch(3)">
  ...
  export default {
    ...
    computed : {
      ...
      workRecord(){
        if(this.workSuccessHint.time){
          ...
          let outRecord = this.attendInfo.outRecord;
          if(outRecord){
            return outRecord;
          }else {
      ...
      leaveRecord(){
        if(this.leaveSuccessHint.time){
          ...
          let outBackRecord = this.attendInfo.outBackRecord;
          if(outBackRecord){
            return outBackRecord;
          }else{
      ...
      showPunchPanel(){
        if(this.showWork === null){
          if((!this.workActive && this.leaveActive) || this.attendInfo.outRecord){
            return false;
          }else{
            return true;
          }
    ...
    components: {
      ...
      en: {
        ...
        'workSign' : 'Outing Punch',
        'leaveSign' : 'Back Punch',
        'punchTime' : 'Punch Time :',
        'workPunch' : 'outing Punch',
        'leavePunch' : 'back Punch',
